...
export function useFundingLists() {
  ...
  const fetchLists = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/funding-lists?userId=${walletAddress}`
      );
      if (!response.ok) {
        throw new Error("Failed to fetch funding lists");
      }
  ...
  const createList = useCallback(
    async (list: Omit<FundingList, "id" | "createdAt" | "updatedAt">) => {
      if (!connected || !wallet || !walletAddress) {
        setError("Wallet not connected");
        return null;
      }
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error("Failed to create funding list");
        }
  ...
  const updateList = useCallback(
    async (list: FundingList) => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error("Failed to update funding list");
        }
  ...
  const deleteList = useCallback(
    async (id: string) => {
      ...
      try {
        const response = await fetch(`/api/funding-lists?id=${id}`, {
          method: "DELETE",
        });

        if (!response.ok) {
          throw new Error("Failed to delete funding list");
        }
